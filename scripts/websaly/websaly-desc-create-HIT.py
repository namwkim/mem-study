...
HIT_URL = "https://study.namwkim.org/websaly/desc"  # Provide the URL that you want workers to sent sent to complete you task
...
DESCRIPTION = ("This HIT should take about 5 minutes to complete. In this HIT, you will be presented with a series of images containing web pages. "+
...
KEYWORDS = ['Easy', 'Web Page', 'Text', 'Click', 'Image', 'Describe']
...
BASE_URI = "/images/websaly-db-pilot/targets/"
BASE_URI_BLUR = "/images/websaly-db-pilot/targets_blurred/"
#######################################
...
def create_hits(keyfile, blockfile):
	...
	targets = []
	for root, dirs, files in os.walk("../../public/images/websaly-db-pilot/targets"):
		for file in files:
			if file.startswith('.'):
				continue
			targets.append(file)
	...
	targets_blurred = []
	for root, dirs, files in os.walk("../../public/images/websaly-db-pilot/targets_blurred"):
		for file in files:
			if file.startswith('.'):
				continue
