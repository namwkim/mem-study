...
HIT_URL = "https://study.namwkim.org/osie/move"  # Provide the URL that you want workers to sent sent to complete you task
...
BASE_URI = "/images/osie-db-pilot/targets/"
BASE_URI_BLUR = "/images/osie-db-pilot/targets_blurred/"
#######################################
...
def create_hits(keyfile, blockfile):
	...
	targets = []
	for root, dirs, files in os.walk("../../public/images/osie-db-pilot/targets"):
		for file in files:
			if file.startswith('.'):
				continue
			targets.append(file)
	...
	targets_blurred = []
	for root, dirs, files in os.walk("../../public/images/osie-db-pilot/targets_blurred_30"):
		for file in files:
			if file.startswith('.'):
				continue
